...
## Dietary Habits and Diet Behavior
```{r echo = FALSE}
ggplot(data, aes(Diet, `Climate Crisis Behavior Diet`)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90))
kruskaldt(data, 'Climate Crisis Behavior Diet', 'Diet')
wtdd <- pairwise.wilcox.test(data$`Climate Crisis Behavior Diet`,data$Diet, p.adjust="bonferroni", paired = FALSE)
wtdd$p.value %>%
  datatable(caption = 'p-values of Pairwise Wilcoxon Rank Sum Tests adjusted using Bonferroni correction',
            autoHideNavigation = TRUE,
...
There is a strong relationship between the reported diet behavior and the reported diet style. 
...
## Distance Driven Privately and Driving Behavior
```{r echo = FALSE}
ggplot(data, aes(`Distance Driven Private`, `Climate Crisis Behavior Driving`)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90))
kruskaldt(data, 'Climate Crisis Behavior Driving', 'Distance Driven Private')
wtdd2 <- pairwise.wilcox.test(data$`Climate Crisis Behavior Driving`,data$`Distance Driven Private`, p.adjust="bonferroni", paired = FALSE)
wtdd2$p.value %>%
  datatable(caption = 'p-values of Pairwise Wilcoxon Rank Sum Tests adjusted using Bonferroni correction',
            autoHideNavigation = TRUE,
            options = list(pageLength = nrow(wtdd2$p.value),
                           ordering = FALSE)) %>%
    formatRound(1:ncol(wtdd2$p.value),
              digits = 3)
```

People who report to have not driven at all show significantly higher behavior than all other groups.
Other than that, most other groups are rather similar.
