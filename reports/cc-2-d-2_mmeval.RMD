---
title: "Climate Crisis Iteration 2-d-2 (General) Measurement Model Evaluation"
output: 
...
bootfsmodel <- tar_read("cc_model_2_d_2_fs_boot")
ralist <- tar_read("cc_model_2_d_2_ra")
# Create summaries
# summo <- summary(model)
sumfs <- summary(model$first_stage_model)
# model summary throws error, substitute with first stage model summary
summo <- sumfs
...
The weight of CCIN1 is insignificant on the basis of the bootstrap confidence interval, however, the loading is sufficiently large.
...
The weights for Distrusting Beliefs Integrity and Injunctive Norm are non-significant, but the loadings are sufficiently large.
Therefore they can be retained.
All other weights are significant and relevant.

## Results of the mode B higher-order constructs evaluation

All mode B higher-order constructs meet expectations and can be retained.
...
write_excel_csv(refevalmm[,c(1:3,5:7,4)] %>% filter(Calpha < 1) %>% mutate(across(c(3:7),  round, 2)), "paper/mma_cc-2-d-2.csv",delim = ";")
write_excel_csv(forevalmm[,c(1:2,4:8,10)] %>% mutate(across(c(3:8),  round, 2)), "paper/mmb_cc-2-d-2.csv", delim = ";")
write_excel_csv(forhocevalmm[,-7] %>% mutate(across(c(3:7),  round, 2)), "paper/mmhocb_cc-2-d-2.csv", delim = ";")
```
