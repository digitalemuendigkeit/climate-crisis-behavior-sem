---
title: "Climate Crisis Iteration 2-c-2 (Driving) Measurement Model Evaluation"
site: 
...
bootfsmodel <- tar_read("cc_model_2_c_2_fs_boot")
ralist <- tar_read("cc_model_2_c_2_ra")
# Create summaries
summo <- summary(model)
...
The weight for Distrusting Beliefs Integrity is non-significant, but with a sufficiently large loading, the construct can be retained.
...
write_excel_csv(refevalmm[,c(1:3,5:7,4)] %>% filter(Calpha < 1) %>% mutate(across(c(3:7),  round, 2)), "paper/mma_cc-2-c-2.csv",delim = ";")
#write_excel_csv(forevalmm[,c(1:2,4:8,10)] %>% mutate(across(c(3:8),  round, 2)), "paper/mmb_cc-2-c-2.csv", delim = ";")
write_excel_csv(forhocevalmm[,-7] %>% mutate(across(c(3:7),  round, 2)), "paper/mmhocb_cc-2-c-2.csv", delim = ";")
```
