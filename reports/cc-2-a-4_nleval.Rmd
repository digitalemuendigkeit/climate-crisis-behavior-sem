---
title: "Climate Crisis Iteration 2-a-4 (Diet) Nonlinearity Evaluation"
output:
...
bootprmodel <- tar_read("cc_model_2_a_4_proxy_boot")
# Make summaries
summo <- summary(model)
...
resetdf %>%
  ...
  formatRound(c(3:4),
              3)
```
Using p = 0.05 as a threshold, there is a potential nonlinear effect in the partial regression equation for the influence of Response Beliefs on Behavioral Intention.

# Test alternate model with quadratic effects
...
mobi1altex <- estimate_pls(
  ...
  structural_model = relationships(
    paths(from = c("Distrusting Beliefs"), to = "Behavioral Intention")
  )
)
...
For the alternate model with Response Beliefs as a quadratic influence, the effect size is negligible, even though the t-statistics points to a significant (if negative) effect.
This points to a linear effect of Response Beliefs on Behavioral Intention.

# Summary
We did not find evidence for a quadratic influence of any of the exogenous variables.
