---
title: "Climate Crisis Iteration 2-a-3 (Diet) Measurement Model Evaluation"
output: 
...
bootfsmodel <- tar_read("cc_model_2_a_3_fs_boot")
ralist <- tar_read("cc_model_2_a_3_ra")
# Create summaries
summo <- summary(model)
...
All measurement model parts meet quality criteria.
Therefore, structural model evaluation can commence.

```{r table-getter, echo = FALSE, include = FALSE}
write_excel_csv(refevalmm[,c(1:3,5:7,4)] %>% filter(Calpha < 1) %>% mutate(across(c(3:7),  round, 2)), "paper/mma_cc-2-a-3.csv",delim = ";")
#write_excel_csv(forevalmm[,c(1:2,4:8,10)] %>% mutate(across(c(3:8),  round, 2)), "paper/mmb_cc-2-a-3.csv", delim = ";")
write_excel_csv(forhocevalmm[,-7] %>% mutate(across(c(3:7),  round, 2)), "paper/mmhocb_cc-2-a-3.csv", delim = ";")
```
