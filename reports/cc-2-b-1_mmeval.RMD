---
title: "Climate Crisis Iteration 2-b-1 (Heating) Measurement Model Evaluation"
site: 
...
model <- tar_read("cc_model_2_b_1")
bootmodel <- tar_read("cc_model_2_b_1_boot")
bootfsmodel <- tar_read("cc_model_2_b_1_fs_boot")
ralist <- tar_read("cc_model_2_b_1_ra")
# Create summaries
summo <- summary(model)
...
The weight of CCIN1 is insignificant on the basis of the bootstrap confidence interval, however, the loadings is sufficiently large.
...
The weight for Perceived Response Costs is non-significant on the basis of the bootstrap confidence interval, and the loading is not sufficiently large.
Therefore, the construct should be removed.
...
For Injunctive Norm, the weight is negative, not relevant, and the loading is bordering the 0.5 threshold.
Therefore, it should also be removed as a lower-order construct.
All other weights are significant and relevant.

## Results of the mode B higher-order constructs evaluation

Perceived Response Costs and Injunctive Norm have to be removed, otherwise the mode B higher-order constructs can be retained as-is.

# Results summary

Perceived Response Costs and Injunvtive Norm have to be removed as a lower-order constructs.
