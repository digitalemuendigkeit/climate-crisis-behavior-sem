---
title: "Climate Crisis Iteration 2-a-4 (Diet) Structural Model Evaluation"
output: 
...
bootmodel <- tar_read("cc_model_2_a_4_boot")
sumplspre <- tar_read("cc_model_2_a_4_plspre")
# Make summaries
summo <- summary(model)
...
```
There are no VIF values available - likely because there is only one exogenous construct for each endogenous construct.
...
All path coefficients are significant and all effects over the minimum threshold.
However, the adjusted R² for Response Beliefs is larger in the previous model iteration (cc-2-a-3), which is why that one should be preferred.
