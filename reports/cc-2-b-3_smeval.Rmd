---
title: "Climate Crisis Iteration 2-b-3 (Heating) Structural Model Evaluation"
output: 
...
sumplspre <- tar_read("cc_model_2_b_3_plspre")
# Make summaries
summo <- summary(model)
...
Other than that, all VIF values are below 1.4.
...
The R² value for Behavioral Intention is above 0.25, indicating weak in-sample predictive power.
...
All effect sizes are above 0.02.
...
The structural model can be retained as is.
...
<!-- write_excel_csv(sumsm[,-5] %>% mutate(across(c(3:5,7:8),  round, 2)) %>% mutate(across(c(6),  round, 3)), "paper/smeval-cc-2-b-3.csv",delim = ";") -->
<!-- ``` -->
