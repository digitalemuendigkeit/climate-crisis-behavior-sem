---
title: "Climate Crisis Iteration 2-d-1 (General) Measurement Model Evaluation"
site: 
...
bootfsmodel <- tar_read("cc_model_2_d_1_fs_boot")
ralist <- tar_read("cc_model_2_d_1_ra")
# Create summaries
summo <- summary(model)
...
The weights for Perceived Self-Efficacy and Perceived Response Efficacy are non-significant, and the loadings are not sufficiently large.
Therefore, Response Beliefs as a higher-order construct should be removed and replaced by a Perceived Response Costs construct.
The weight for Distrusting Beliefs Integrity is also non-significant, but the loading is sufficiently large.
Therefore it can be retained.
All other weights are significant and relevant.

## Results of the mode B higher-order constructs evaluation

Response Beliefs should be replaced by a Perceived Response Costs construct, otherwise the mode B higher-order constructs can be retained as-is.

# Results summary

Response Beliefs should be replaced by a Perceived Response Costs construct.
